...
void f(string inputfile,string outputfile)
{
    ...
    //该函数能够将 string 字符串转换为C风格的字符串，并返回该字符串的 const 指针（const char*）
	{
		...
		outedit<<"pitch"<<','<<"roll"<<','<<"yaw"<<','<<"gyroX_bias"<<','<<"gyroY_bias"<<','<<"gyroZ_bias"<<','<<endl;//输出表格的第一行定义
		...
		while (fgets(line, MAX_LINE, fp))
		{
			...
			accX= atof(strtok( line, delims ));//m/s^2
			accY= atof(strtok( NULL, delims ));
			accZ= atof(strtok( NULL, delims ));

			gyroX = atof(strtok( NULL, delims ));//rad/s
            ...
            outedit<<pitch<<','<<roll<<','<<yaw<<','<<wx_bias<<','<<wy_bias<<','<<wz_bias;
